...
def get_each_dataset(
    ...
    if dataset_config["dataset_type"] == "m3it":
        ...
        train_dataset = LlavaDataset.create(
            dataset_config, processor, max_length, "train"
        )
        val_dataset = LlavaDataset.create(
            dataset_config, processor, max_length, "validation"
        )

    elif dataset_config["dataset_type"] == "soccernet":
        train_dataset = SoccerNetDataset.create(
            dataset_config, processor, max_length, "train"
        )
        val_dataset = SoccerNetDataset.create(
            dataset_config, processor, max_length, "validation"
        )

    else:
        raise ValueError(
            f"dataset_type: {dataset_config['dataset_type']} is not supported."
        )
...
def get_dataset(config: Dict) -> Tuple[Dataset, Dataset]:
    ...
    for dataset_config_path in config["dataset_config_path"]:
        ...
        train_dataset, val_dataset = get_each_dataset(
            dataset_config, processor, max_length
        )
